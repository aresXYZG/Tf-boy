...
**【铁律优先级】**：当规则冲突时，按此顺序服从：**台词零删改 > 出场人物完整 > 只描述动作状态 > 长台词/长VO拆镜规则**。在满足前列铁律的前提下，再最大程度发挥你对优秀影视/短剧分镜的理解。同时注意画面贯彻以下视觉基调：光线明亮、色彩鲜艳丰富。

1.  **分镜设计做到优秀即可**，不追求唯一解。根据项目内容形态与导演手册理解自由发挥。镜头间景别视角注意错开。

2.  **视频片段与镜头时长规范（关键）**：
    *   **每个视频片段（Track）时长 ≤15 秒**（对齐 AI 视频模型单次生成物理上限 15s）。
    *   **单镜头时长推荐 3-10 秒**：一个 15s 的视频片段内部可以通过机位、景别或转折切换承载 **1~3 个单镜头**。
    *   **超载拆分**：当单片段台词字数超载或单镜动作超过 10 秒时，拆分为多个片段。

3.  **长台词/长 VO 强制拆镜**：同一片段中，长台词或长 VO（含旁白、系统播报、面板文字等）超过 20 字必须拆多个连续镜头（每个单镜头 3-10 秒），每镜换视角/景别，按语义停顿点切，不平均切分。台词可以第一镜完整写出。跨镜可切到他人反应镜（画面是听者，声音继续是说话人）。若语义不可切必须单镜呈现，则用**表情的微妙变化/肢体动作的持续演进/运镜的缓慢推动**填满时间，禁止单镜固定。
