...
- **内容形态决定制作技能**：项目的 `contentFormat` 是制作阶段技能路由键。阶段1导演规划和阶段4分镜表会自动叠加 `content_formats/<contentFormat>/` 下的专属覆盖技能；不得把其他形态的时长、分集、卡点、投流或解说规则混入当前项目。通用层只定义跨形态制作契约。
...
- **形态隔离**：`vertical_episode` 才可使用付费点、投流素材和短剧集末规则；`series_drama` 使用自然集间承接；`single_film` 使用全片闭环；`explainer_video` 使用知识结构、证据边界和视觉功能。任何形态都不得把另一形态的商业规则带入当前任务。
---
...
| 项 | 说明 |
|----|------|
| 派发 | 执行层依据当前剧本制定导演拍摄计划 |
| 输出 | 导演拍摄计划；执行层同步到前端 |
| 前置条件 | 剧本和资产已存在于工作区 |
| 审核 | 不需要 |
| 形态规则 | 由通用导演规划 + 当前 `contentFormat` 覆盖层共同决定 |
...
| 项 | 说明 |
|----|------|
| 派发 | 执行层依据剧本、导演规划和当前形态覆盖层生成结构化分镜表 |
| 输出 | 结构化分镜表（执行层保存） |
| 质量门 | 分镜拆分粒度合理、字段完整、台词/VO完整、关联资产真实、Track时长合规 |
| 前置条件 | 阶段1（导演规划）已完成；衍生资产相关阶段（阶段2/3）按需完成 |
| 审核 | **需要** → 执行完毕后自动派发监督层 |

**阶段特有约束：** `associateAssetsIds` 中的每一个值必须是资产库中实际存在的资产 ID，不是数组索引；缺失基础资产不转化为审核问题。
...
- 阶段1审核重点：场次/结构单元覆盖、台词统计、情绪或认知功能、过渡必要性、资产与下游可执行性。
- 阶段4审核重点：剧本覆盖、台词/VO忠实、分镜字段完整、真实资产 ID、Track 时长、动作与空间连续，以及当前形态的专属质量要求。
```
...
项目形态：{contentFormat}
上下文：{必要数据摘要}
形态要求：只执行当前项目形态覆盖规则，不引用其他形态的商业或结构模板。
```
