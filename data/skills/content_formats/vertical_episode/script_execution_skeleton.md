...
- **内容形态自适应（核心原则）**：
  - **超短剧（vertical_episode，55s/集）**：必须执行【二、前10集黄金结构】、【三、付费点（卡点）设置规范】（10%/30%/50%/70%/90%）与投流素材爆点设计。
  - **中长连续剧（series_drama，5-15分钟/集）**：按长剧三幕/多幕大纲推进，保留集末自然悬念与人物弧光，**跳过付费卡点与超短剧投流公式**。
  - **短故事 / 微电影（single_film，整片单集完结）**：单集完整闭环叙事，**时长根据原文实际字数、情节事件密度、人物对话量与叙事节奏动态推算得出最合理的真实总时长**，按整片总时长规划起承转合，**跳过分集与付费卡点**。
- 总时长 = 集数 × 单集时长（从【项目配置】读取，禁止硬编码）
- 压缩比 ≤ 40%
- 每集必须有集末钩子（整片单集完结形态除外）
- 付费策略按【项目配置】与内容形态执行
