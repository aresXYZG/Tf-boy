...
export default router.post("/", validateFields(requestSchema), async (req, res) => {
  ...
  if (base64) {
    ...
    // 查询资产中持久化的 roleMeta
    const assetRecord = await u.db("o_assets").where("id", id).select("roleMeta").first();
    ...
    if (roleMeta && roleMeta.species !== undefined && roleMeta.species !== 1) {
      ...
      const faceSample = roleMeta ? await sampleAssetReferences(roleMeta, name) : await sampleDualFaceAssets(prompt, name);
  ...
  const userPrompt = buildPrompt(cfg, project.artStyle!, name, prompt, faceSummary, isNonHumanRole, roleMeta);
